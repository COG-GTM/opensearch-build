...
| `post { unstable { ... } }` and `currentBuild.result = 'UNSTABLE'` | `distribution-build.jenkinsfile:1044-1052, 1067-1073` | **No equivalent.** Actions has only success/failure/cancelled. `markStageUnstableIfPluginsFailedToBuild()` (a pipeline-local helper, not a library step) greps stage logs for `Error building` and downgrades the stage. The port keeps `continue-on-error` at the component level and surfaces the failed plugins as `::warning::` annotations in the build summary; if "unstable" must be actionable, a check run with a neutral conclusion is the closest model. |
